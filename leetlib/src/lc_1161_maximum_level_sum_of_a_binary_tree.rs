...
impl Solution {
    pub fn max_level_sum(root: Node) -> i32 {
        fn solve(node: Node) -> i32 {
            ...
            1 + t.iter().position(|&x| x == max).unwrap() as i32
        }
        solve(root)
    }
}
...
mod tests {
    ...
    #[test]
    fn ex1() {
        let input = TreeNode::from_string("1,7,0,7,-8".to_string());
        assert_eq!(Solution::max_level_sum(input), 2);
    }

    #[test]
    fn ex2() {
        let input =
            TreeNode::from_string("989,null,10250,98693,-89388,null,null,null,-32127".to_string());
        assert_eq!(Solution::max_level_sum(input), 2);
    }
}
